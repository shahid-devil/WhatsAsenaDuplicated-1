...
if (Config.WORKTYPE === 'private') {
  ...
  Asena.addCommand({pattern: 'animepic ?(.*)', fromMe: true, desc: pic, usage: 'animepic normal // animepic nsfw'}, (async (message, match) => {
  if (match[1] == 'normal') {
    ...
    await message.sendMessage(Buffer.from(image_buffer.data), MessageType.image, { caption: 'Hecho por *Skueletor*' })
    ...
    await message.sendMessage(Buffer.from(image_buffer_nsfw.data), MessageType.image, { caption: 'Hecho por *Skueletor*' })
  } else {
    return await message.client.sendMessage(message.jid,wr_usage,MessageType.text)
  }
}));
Asena.addCommand({pattern: 'animegif ?(.*)', fromMe: true, desc: giff, usage: 'animegif normal // animegif nsfw'}, (async (message, match) => {
  if (match[1] == 'normal') {
    var gif_link = await WhatsAsenaStack.anime_gif('normal')
    var gif_buffer = await axios.get(gif_link, { responseType: 'arraybuffer' })
    await message.sendMessage(Buffer.from(gif_buffer.data), MessageType.video, { caption: 'Hecho por *Skueletor*', mimetype: Mimetype.gif })
  } else if (match[1] == 'nsfw') {
    ...
    await message.sendMessage(Buffer.from(gif_buffer_nsfw.data), MessageType.video, { caption: 'Hecho por *Skueletor*', mimetype: Mimetype.gif })
  } else {
    return await message.client.sendMessage(message.jid,wr_usage,MessageType.text)
  }
}));
...
if (Config.WORKTYPE === 'public') {
  ...
  Asena.addCommand({pattern: 'animepic ?(.*)', fromMe: false, desc: pic, usage: 'animepic normal // animepic nsfw'}, (async (message, match) => {
  if (match[1] == 'normal') {
    var image_link = await WhatsAsenaStack.anime_wall('normal')
    var image_buffer = await axios.get(image_link, { responseType: 'arraybuffer' })
    await message.sendMessage(Buffer.from(image_buffer.data), MessageType.image, { caption: 'Hecho por *Skueletor*' })
...
Asena.addCommand({pattern: 'animegif ?(.*)', fromMe: false, desc: giff, usage: 'animegif normal // animegif nsfw'}, (async (message, match) => {
  if (match[1] == 'normal') {
    var gif_link = await WhatsAsenaStack.anime_gif('normal')
    var gif_buffer = await axios.get(gif_link, { responseType: 'arraybuffer' })
    await message.sendMessage(Buffer.from(gif_buffer.data), MessageType.video, { caption: 'Hecho por *Skueletor*', mimetype: Mimetype.gif })

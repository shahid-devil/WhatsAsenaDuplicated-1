...
Asena.addCommand({pattern: 'fb ?(.*)', fromMe: true, desc: CLR_DESC, usage: 'fb https://www.facebook.com/100057026936893/videos/578822506615264'}, (async (message, match) => {
...
Asena.addCommand({pattern: 'fb ?(.*)', fromMe: false, desc: CLR_DESC, usage: 'fb https://www.facebook.com/100057026936893/videos/578822506615264'}, (async (message, match) => {
  var reg = new RegExp(/^http(?:s?):\/\/(?:www\.|web\.|m\.)?facebook\.com\/([A-z0-9\.]+)\/videos(?:\/[0-9A-z].+)?\/(\d+)(?:.+)?$/, 'gm')
  var is_valid = reg.test(match[1])
  if (!is_valid) return await message.client.sendMessage(message.jid, wr, MessageType.text)
  var payload = await WhatsAsenaStack.facebook(match[1])

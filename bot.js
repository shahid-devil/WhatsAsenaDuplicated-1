...
const WhatsAlexaDB = config.DATABASE.define('WhatsAsenaDuplicated', {
    info: {
...
async function whatsAsena () {
    ...
    conn.on('connecting', async () => {
        console.log(`${chalk.green.bold('Skueletor')}
${chalk.white.bold('Version:')} ${chalk.red.bold(config.VERSION)}

${chalk.blue.italic('Made By Skueletor')}`);
    });
    ...
    conn.on('open', async () => {
        ...
        console.log(
         ...
         if (config.LANG == 'ES') {
             await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/gif/WhatsAsena-Thumb.jpeg"), MessageType.image, { caption: `『 Skueletor 』\n\n¡Hola ${conn.user.name}!\nAhora puedes usar el bot en cualquier chat :)\n\n*🆘 ¡Ayuda general para usted! 🆘*\n\n🔹 */alive:* Revise si el bot está funcionando.\n\n🔹 */commands:* Muestra la lista completa de los comandos existentes\n\n🔹 */restart:* Reinicia el bot (en caso de algún error).\n\nGracias por comprar el bot Skueletor 💖`});
             
         } else if (config.LANG == 'ID') {
             await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/gif/WhatsAsena-Thumb.jpeg"), MessageType.image, { caption: `『 WhatsAlexa 』\n\nHalo ${conn.user.name}!\n\n*🆘 Bantuan umum 🆘*\n\n🔹 *#alive:* Periksa apakah bot sedang berjalan.\n\n🔹 *#list:* Menampilkan daftar lengkap perintah.\n\n🔹 *#restart:* Ini me-restart bot.\n\n🔹 *#shutdown:* Ini Matikan/Matikan bot.\n\n *⚠ Peringatan, Jika Anda mematikan/mematikan, tidak ada perintah untuk menghidupkan bot Jadi Anda harus pergi ke heroku & Nyalakan worker. ⚠*.\n\nTerima Kasih Telah Menggunakan WhatsAlexa 💖`});
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'ES') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/gif/WhatsAsena-Thumb.jpeg"), MessageType.image, { caption: '*『 ERROR 』*\n\n*Ha ocurrido un error en el bot Skueletor.*\n_¡Informe este error al desarrollador! [ Skueletor ]._\n\n*Error:* ```' + error + '```\n\n' });
                                
                            } else if (config.LANG == 'ML') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/gif/WhatsAsena-Thumb.jpeg"), MessageType.image, { caption: '*『 പിശക് 』*\n\n*WhatsAlexa പിശക് സംഭവിച്ചു!*\n_ഈ പിശക് ഡെവലപ്പറെ അറിയിക്കുക! [ MrChaby ]._\n\n*പിശക്:* ```' + error + '```\n\n' });
                                
                            } else {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/gif/WhatsAsena-Thumb.jpeg"), MessageType.image, { caption: '*『 KESALAHAN 』*\n\n*WhatsAlexa telah terjadi kesalahan!*\n_Laporkan kesalahan ini ke pengembang [ MrChaby ]._\n\n*Kesalahan:* ```' + error + '```\n\n' });
                            }
                        }
                    }
                }

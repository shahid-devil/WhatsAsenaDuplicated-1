...
${chalk.blue.italic('Made By TOXIC-DEVIL')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
         ...
         if (config.LANG == 'ES') {
             await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/gif/WhatsAsena-Thumb.jpeg"), MessageType.image, { caption: `『 WhatsAlexa 』\n\nHola ${conn.user.name}!\n\n*🆘 ¡Esta es una ayuda general para tí! 🆘*\n\n🔹 */alive:* Revisa si el bot está funcionando correctamente.\n\n🔹 */commands:* Muestra la lista completa de los comandos.\n\n🔹 */restart:* Reinicia el bot (en caso de algún problema)\n\n🔹 */shutdown off:* Este comando apaga el bot.\n\n *⚠ Advertencia, Si apagas el bot no existe ningún otro comando para volver a encenderlo, debe contactar a Skueletor para volver a activarlo manualmente. ⚠*.\n\n*Gracias por usar Skueletor Bot* 💖`});
    ...
    setInterval(async () => { 
        if (config.AUTOBIO == 'true') {
            ...
            else if (conn.user.jid.startsWith('51')) { 
    ...
    setInterval(async () => { 
        ...
        while (getGMTh == 19 && getGMTm == 1) {
            var announce = ''
            if (config.LANG == 'ES') announce = '📢 ¡El sistema de anuncios ahora se ha agregado a Skueletor Bot!! 🥳\nDiariamente Nosotros (los desarrolladores) anunciaremos *eventos/características/algo nuevo* de este sistema 📝\nMantente conectado ✅'
            ...
            let video = ''
            let image = 'https://i.hizliresim.com/r3sal8f.jpg'
